...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    chart.draw_series(
        hist1.iter().enumerate().map(|(i, &count)| {
            ...
            let x1 = x0 + bin_width * 0.4; // 너비를 0.4로 줄임
    ...
    // 두 번째 히스토그램 막대 그리기 (녹색) - 첫 번째 히스토그램 옆에 배치
    chart.draw_series(
        hist2.iter().enumerate().map(|(i, &count)| {
            let x0 = min_val + i as f64 * bin_width + bin_width * 0.4; // 파란색 막대 바로 옆에서 시작
            let x1 = x0 + bin_width * 0.4; // 같은 너비로 설정
