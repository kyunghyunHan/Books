...
const OUT_FILE_NAME: &str = "./assets/img/ch_7_7.png";
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut chart = ChartBuilder::on(&root)
        ...
        .build_cartesian_2d(min_val..max_val, 0..(max_freq as i32))?;
    ...
    chart
        .draw_series(hist1.iter().enumerate().map(|(i, &count)| {
    ...
    chart
        .draw_series(hist2.iter().enumerate().map(|(i, &count)| {
            let x0 = min_val + i as f64 * bin_width;
