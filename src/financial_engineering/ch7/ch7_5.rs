...
const OUT_FILE_NAME: &str = "./assets/img/ch_7_5.png";
...
fn get_color(value: f64, min_val: f64, max_val: f64) -> RGBColor {
    ...
    if normalized < 0.5 {
        ...
        RGBColor((intensity * 255.0) as u8, (intensity * 255.0) as u8, 255)
    } else {
        // 흰색에서 빨간색으로 (0.5~1.0)
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..COLORBAR_SEGMENTS {
        ...
        color_bar.draw_series(std::iter::once(Rectangle::new(
            [(0, y_min), (1, y_max)],
            color.filled(),
        )))?;
    }
